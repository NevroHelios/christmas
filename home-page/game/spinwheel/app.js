...
function spinWheel() {
    if (!spinning) {
        ...
        // Use transform property to rotate the wheel smoothly
        wheel.style.transition = 'transform 3s ease-out';
        wheel.style.transform = `rotate(${degrees}deg)`; // Use rotate instead of setting the transform directly
...
function applyWeatherEffect(color) {
    ...
    switch (color) {
        case '#ff0000':
            createSnowfall();
            playAudio('285311-Mountain_Wind-rushing_gusts_through_a_snowy_forest-snowfall.wav', 10000); // Stop after 10 seconds
            break;
        case '#00ff00':
            createDrizzle();
            playAudio('150672-Weather_29-Rain_forest_downpour_heavy_steady_loop-SFX_Bible-2011.wav', 10000); // Stop after 10 seconds
            break;
        case '#0000ff':
            createFog();
            playAudio('birdsong-and-crows-in-fog-19064.mp3', 10000); // Stop after 10 seconds
            break;
        case '#ffff00':
            createChillWinds();
            playAudio('smooth-cold-wind-looped-135538.mp3', 10000); // Stop after 10 seconds
            break;
        case '#bbf0f0':
            createIce();
            playAudio('ice-sound.mp3', 10000); // Stop after 10 seconds
            break;
        default:
            break;
    }
...
function createChillWinds() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        windLine.style.animationDuration = `${Math.random() * 5 + 5}s`; // Adjust the duration
...
function playAudio(soundFile, duration) {
    const audio = new Audio(soundFile);
    audio.play();

    // Stop audio after the specified duration
    setTimeout(() => {
        audio.pause();
        audio.currentTime = 0;
    }, duration);
